"""
Plot training metrics from training_history.json for GraphCodeBERT using matplotlib.
Creates comprehensive visualizations of GraphCodeBERT training.
Includes MLM and Edge loss tracking (unlike UniXcoder which only has total loss).
"""
...
def plot_total_loss(history: dict, save_path: str = None):
    ...
    ax.set_title('GraphCodeBERT: Training & Validation Loss', fontsize=14, fontweight='bold')
...
def plot_train_vs_val(history: dict, save_path: str = None):
    ...
    ax.set_xlabel('Epoch', fontsize=12, fontweight='bold')
    ax.set_ylabel('Loss', fontsize=12, fontweight='bold')
    ax.set_title('GraphCodeBERT: Train vs Validation Comparison', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
def plot_mlm_vs_edge(history: dict, save_path: str = None):
    """Plot MLM vs Edge loss comparison."""
    fig, ax = plt.subplots(figsize=(12, 6))
    ...
    ax.plot(epochs, train_mlm, marker='o', linewidth=2.5, label='Train MLM',
            color=colors['train_mlm'], markersize=10)
    ax.plot(epochs, train_edge, marker='^', linewidth=2.5, label='Train Edge',
            color=colors['train_edge'], markersize=10)
    ax.plot(epochs, val_mlm, marker='s', linewidth=2.5, label='Val MLM',
            color=colors['val_mlm'], markersize=10)
    ax.plot(epochs, val_edge, marker='D', linewidth=2.5, label='Val Edge',
            color=colors['val_edge'], markersize=10)

    ax.set_xlabel('Epoch', fontsize=12, fontweight='bold')
    ax.set_ylabel('Loss', fontsize=12, fontweight='bold')
    ax.set_title('GraphCodeBERT: MLM vs Edge Loss', fontsize=14, fontweight='bold')
    ax.legend(fontsize=11, loc='upper right')
    ax.grid(True, alpha=0.3)
...
def plot_batch_losses(history: dict, save_path: str = None):
    ...
    ax1.set_title('GraphCodeBERT: Training Batch Losses - All Iterations', fontsize=13, fontweight='bold')
    ...
    ax2.set_title('GraphCodeBERT: Validation Batch Losses - All Iterations', fontsize=13, fontweight='bold')
...
def plot_learning_rate(history: dict, save_path: str = None):
    ...
    ax.set_title('GraphCodeBERT: Learning Rate Schedule', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=11)
...
def plot_batch_statistics(history: dict, save_path: str = None):
    ...
    plt.suptitle('GraphCodeBERT: Batch Loss Statistics', fontsize=15, fontweight='bold')
    plt.tight_layout()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Plot GraphCodeBERT training metrics from training_history.json')
    ...
    parser.add_argument('--output_dir', type=str, default='graphcodebert_training_plots',
                        help='Directory to save plots')
    parser.add_argument('--all', action='store_true', help='Generate all plots')
    parser.add_argument('--total', action='store_true', help='Plot total loss')
    parser.add_argument('--mlm_edge', action='store_true', help='Plot MLM vs Edge')
    ...
    generate_all = args.all or not any([args.total, args.mlm_edge, args.compare, args.batches, args.lr, args.stats])

    try:
        if args.total or generate_all:
            print("Generating total loss plot...")
            plot_total_loss(history, output_dir / 'graphcodebert_total_loss.png')

        if args.mlm_edge or generate_all:
            print("Generating MLM vs Edge plot...")
            plot_mlm_vs_edge(history, output_dir / 'graphcodebert_mlm_vs_edge.png')

        if args.compare or generate_all:
            print("Generating train vs val comparison...")
            plot_train_vs_val(history, output_dir / 'graphcodebert_train_vs_val.png')

        if args.batches or generate_all:
            print("Generating batch losses plot...")
            plot_batch_losses(history, output_dir / 'graphcodebert_batch_losses.png')

        if args.lr or generate_all:
            print("Generating learning rate plot...")
            plot_learning_rate(history, output_dir / 'graphcodebert_learning_rate.png')

        if args.stats or generate_all:
            print("Generating batch statistics plot...")
            plot_batch_statistics(history, output_dir / 'graphcodebert_batch_statistics.png')

        # Always generate the comprehensive figure
        if generate_all:
            print("Generating comprehensive metrics plot...")
            plot_all_metrics(history, output_dir / 'graphcodebert_all_metrics.png')

        print(f"\n✓ All plots saved to: {output_dir}")
        print("✓ All files have 'graphcodebert_' prefix\n")
